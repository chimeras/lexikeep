...
export default function StudentInboxPage() {
  ...
  const [teachers, setTeachers] = useState<Pick<Profile, 'id' | 'username' | 'avatar_url' | 'role'>[]>([]);
  ...
  // Load initial conversations and teachers
  useEffect(() => {
    if (!userId) return;

    const initData = async () => {
      setLoading(true);
      const [convsRes, teachersRes] = await Promise.all([
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 pt-16">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Loading inbox...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-6xl gap-4 px-4 pt-20 pb-4 md:px-6">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-slate-100 shrink-0">
          <h1 className="font-bold text-lg text-slate-800">Inbox</h1>
          <button
            type="button"
      ...
      >
        {selectedPartner ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50/50">
              {loadingMessages ? (
                ...
                <div className="flex h-full flex-col items-center justify-center text-center text-sm text-gray-400">
                  ...
                  Say hello to {selectedPartner.username}!
                </div>
              ) : (
                messages.map((msg) => {
            ...
            <form onSubmit={handleSend} className="p-3 border-t border-slate-100 flex gap-2 shrink-0 bg-white">
              ...
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                <Send size={16} />
              </button>
            </form>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/20">
            ...
            <h3 className="font-bold text-slate-800 text-base">Your Messages</h3>
            <p className="text-sm text-gray-400 max-w-sm mt-1">
              Select a conversation from the sidebar or start a new chat with one of your teachers.
            </p>
            <button
              type="button"
              onClick={() => setShowNewChatModal(true)}
              className="mt-4 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors shadow-sm"
            >
              Start Conversation
            </button>
          </div>
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 mb-4">
              {teachers.length === 0 ? (
                <div className="py-6 text-center text-sm text-gray-400">
                  No teachers found. Join a class first.
                </div>
              ) : (
                teachers.map((teacher) => (
                  <button
                    key={teacher.id}
                    onClick={() => handleSelectPartner(teacher)}
                  ...
                  >
                    ...
                    <div>
                      <p className="font-semibold text-sm">{teacher.username}</p>
